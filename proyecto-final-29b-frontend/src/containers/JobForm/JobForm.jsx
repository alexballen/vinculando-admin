...
const JobForm = () => {
  ...
  const confirmSubmit = async (values, { resetForm }) => {
    const pictures = await Promise.all(
      values.pictures.map(async (picture) => await uploadImage(picture))
  ...
  return (
    <ScrollView style={tw`mx-3 mt-10 mb-10 p-3 bg-white shadow-md rounded-lg`}>
      <Text style={tw`text-center font-bold mb-3 w-full p-2 text-lg`}>
        Postea una profesión aquí{" "}
      </Text>
      <Formik
      ...
      >
        {({
          ...
          const { description, availableDays, pictures, name, pricing } =
            values;
            ...
            <>
              ...
              <FormCalendar
                ...
                onSelect={(selectedDates) =>
                  setFieldValue("availableDays", selectedDates)
                }
              />
