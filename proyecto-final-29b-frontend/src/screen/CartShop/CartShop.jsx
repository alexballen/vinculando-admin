  ...
  return (
    <ScrollView>
      ...
      <ScrollView style={tw`h-full bg-gray-200`}>
        <Text style={tw`p-3 text-5 text-center`}>
          Carrito({shopList.length})
        </Text>
        <ScrollView style={tw`h-screen`}>
          {shopList.length > 0 ? (
            shopList.map((item) => (
              <View key={item.id} style={tw`bg-[#F1F1F1] mx-5 my-2`}>
                <Text style={tw`p-1 text-center font-bold border-b`}>
                  {item.name}
                </Text>
                <View style={tw`p-2`}>
                  ...
                  <View style={tw`flex justify-center w-full`}>
                    {item.pictures?.length > 0 ? (
                      item.pictures.map((pic, i) => (
                        <Image
                          key={i}
                          style={tw`h-25 w-25 m-1`}
                          source={{ uri: pic }}
                        />
                      ))
                    ) : (
                      <View
                        style={tw`w-100 h-25 flex align-center justify-center`}
                      >
                  ...
                  >
                    <Text
                      style={tw`text-center text-base text-white bg-blue-400 rounded-lg w-35 p-1`}
                    >
                      Eliminar
                    </Text>
                  </TouchableOpacity>
        ...
        {shopList.length > 0 ? (
          <View>
            <View
              style={tw`flex flex-row justify-between items-center w-screen m-3 p-3 border-b border-blue-400`}
            >
            ...
            >
              <Text
                style={tw`text-center text-base text-white bg-blue-400 rounded-lg w-40 p-4`}
              >
                Finaliza la compra
              </Text>
            </TouchableOpacity>
